...
def evaluate_detailed_metrics(eval_df, field_labels_mappings, results_directory):
    ...
    for field, top_values in field_labels_mappings.items():
        ...
        # precision, recall, f1 for all values 
        precision, recall, f1, _ = precision_recall_fscore_support(gt_values, pred_values
                                                                   , labels=list(top_values)
        ...
        # Store results
        metrics[field] = {
             'precision': dict(zip(top_values, precision)),
            'recall': dict(zip(top_values, recall)),
            'f1_score': dict(zip(top_values, f1))
            # 'confusion_matrix': conf_matrix
        }

        def save_metrics_to_csv(metrics, results_directory):
            # Flatten the nested metrics dictionary into a list of dictionaries suitable for DataFrame conversion
            flat_list = []
            for field, values in metrics.items():
                for metric_type, metric_values in values.items():
                    for label, score in metric_values.items():
                        flat_list.append({
                            'Field': field,
            ...
            df.to_csv(f"{results_directory}/metrics_detailed.csv", index=False)
    ...
    # pd.DataFrame(metrics).to_csv(f"{results_directory}/metrics_detailed.csv")
    save_metrics_to_csv(metrics, results_directory)

    return metrics

def read_metrics_from_file(file_path):
    """Read metrics from a CSV file and return a DataFrame."""
    df = pd.read_csv(file_path)
    return df

def concatenate_metrics(root_directory):
    # Dictionary to store data from each model (directory)
    model_data = {}

    # Loop through all directories in the root directory
    for model_name in os.listdir(root_directory):
        model_dir = os.path.join(root_directory, model_name)
        if os.path.isdir(model_dir):
            # Assume there's one main metrics file per directory, adjust the pattern as needed
            file_path = os.path.join(model_dir, 'metrics_detailed.csv')
            if os.path.exists(file_path):
                df = read_metrics_from_file(file_path)
                # Ensure the DataFrame is indexed by Field, Metric Type, and Label
                df.set_index(['Field', 'Metric Type', 'Label'], inplace=True)
                model_data[model_name] = df['Score']

    # Concatenate all data into a single DataFrame along the columns
    combined_df = pd.concat(model_data, axis=1)
    combined_df.columns.names = ['Model']  # Naming the column level for clarity
    return combined_df
...
def main():
    ...
    for model_store_path in file_name_dict.values():
        ...
        # Concatenate detailed metrics previously generated into one single df 
        root_directory = 'eval_results'
        final_df = concatenate_metrics(root_directory)
        final_df.to_csv(f'{root_directory}/detailed_metrics_concatenated.csv')
